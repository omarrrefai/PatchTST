  ...
  return (
    <div className="card">
      ...
      <div className="grid" style={{ gridTemplateColumns: "1fr 1fr", marginTop: 8 }}>
        <div className="subtle">Load</div>
        <div className="subtle" style={{ textAlign: "right" }}>
          {(loadMW ?? 0).toFixed(2)} MW
        </div>

        <div className="subtle">Price (F)</div>
        <div className="subtle" style={{ textAlign: "right" }}>
          ${(priceF ?? 0).toFixed(2)}/MWh
        </div>

        <div className="subtle">Price (A)</div>
        <div className="subtle" style={{ textAlign: "right" }}>
          ${(priceA ?? 0).toFixed(2)}/MWh
        </div>

        <div className="subtle">Prediction</div>
        <div className="subtle" style={{ textAlign: "right" }}>
          {src ?? "?"} · {windowLabel ?? "unknown"}
        </div>

        <div className="subtle">SOC</div>
        <div className="subtle" style={{ textAlign: "right" }}>
          {(socPct ?? 0).toFixed(1)}%
        </div>
      </div>
...
export default function App() {
  ...
  if (!data || data.status === "starting") {
    return (
      <div className="container">
        <span className="badge">Starting… waiting for first step</span>
      </div>
    );
  }
  ...
  const loadSeries = [
    { name: "Total Load (MW)", data: labels.map((x, i) => ({ x, y: totalLoad[i] ?? null })) },
  ];
  const savingsSeries = [
    { name: "Savings (forecast $)", data: labels.map((x, i) => ({ x, y: totalSavings[i] ?? null })) },
  ];
  const priceSeries = AREAS.map((a) => ({
    name: `Price ${a} (F)`,
    data: (priceHist[a] ?? []).map((y, i) => ({ x: labels[i], y })),
  }));
  const perAreaSeries = AREAS.map((a) => ({
    name: `Load ${a} (MW)`,
    data: (areaLoadHist[a] ?? []).map((y, i) => ({ x: labels[i], y })),
  }));
  const modeSeries = ["short", "medium", "long"].map((m, idx) => ({
    name: m,
    data: modeHist.map((v, i) => ({ x: labels[i], y: v === m ? idx + 1 : null })),
  }));
    ...
    <div className="container">
      <div className="header" style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <span className="badge">
          {running ? "Running" : data.status} — step {step + 1}
        </span>
      ...
      {page === "comparison" ? (
        ...
        <>
          ...
          <div className="grid" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(230px, 1fr))" }}>
            {AREAS.map((a) => (
              <AreaCard
                ...
                priceA={priceA[a]}
                socPct={soc[a]}
                src={forecastSource[a]}
                windowLabel={predictionWindow[a]}
              />
            ))}
          </div>
          ...
          <div className="grid grid-2">
            <div className="card">
              ...
              <div style={{ marginTop: 10, overflowX: "auto" }}>
                <table className="table">
                  <thead>
                    <tr><th>Area</th><th>Forecast</th><th>Actual</th><th>Source</th><th>Window</th></tr>
                  </thead>
                  <tbody>
                    {AREAS.map((a) => (
                      <tr key={a}>
            ...
            {previous && (
              <div className="card">
                <div className="title">Previous step source/window</div>
                <div style={{ marginTop: 10, overflowX: "auto" }}>
                  <table className="table">
                    <thead>
                      <tr><th>Area</th><th>Source</th><th>Window</th></tr>
                    </thead>
                    <tbody>
                      {AREAS.map((a) => (
                        <tr key={a}>
